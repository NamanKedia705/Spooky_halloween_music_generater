...
class SequencerApp:
    def __init__(self, manager):
        self.selected_demo = 0  # Default demo (index 0)  
        ...
        self.demo_play_count = 0
        self.max_demo_plays = 3
        ## correct matrix
        # Correct patterns for each demo (3 instruments × 8 steps)
        self.correct_patterns = {
            0: [  # Demo 1 pattern
                [True, False, True, False, True, False, True, False],   # Bass
    ...
    def create_gui_elements(self):
        ...
        # Random Demo button
        demo_random_rect = pygame.Rect(420, 350, 160, 40)
        self.demo_random_button = pygame_gui.elements.UIButton(
            relative_rect=demo_random_rect,
            text="Random Demo",
            manager=self.manager
        )

        # Play Demo button
        play_demo_rect = pygame.Rect(600, 350, 160, 40)
        self.play_demo_button = pygame_gui.elements.UIButton(
            relative_rect=play_demo_rect,
            text="Play Demo",
            manager=self.manager
        )
        #check button
        check_button_rect = pygame.Rect(420, 460, 160, 40)
        self.check_button = pygame_gui.elements.UIButton(
            relative_rect=check_button_rect,
            text="Check My Beat",
            manager=self.manager
        )
        
        # Label to show which demo is selected
        demo_label_rect = pygame.Rect(420, 300, 340, 30)
        self.demo_label = pygame_gui.elements.UILabel(
            relative_rect=demo_label_rect,
            text="Demo: None",
            manager=self.manager
        )

        # Stop Demo button
        stop_demo_rect = pygame.Rect(600, 400, 160, 40)
        self.stop_demo_button = pygame_gui.elements.UIButton(
            relative_rect=stop_demo_rect,
            text="Stop Demo",
            manager=self.manager
        )
        
    def load_sounds(self):
        """Loads sound effects and demo tracks."""
        self.sounds = {
        ...
        # Demos: full mixed tracks (drums + backing together)
        self.demo_tracks = [
            "sounds/demo1.wav",
            "sounds/demo2.wav",
            "sounds/demo3.wav"
        ...
        # Spooky melodies (used in Halloween mode)
        self.spooky_melodies = [
        ...
        if event.type == pygame.USEREVENT:
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                ...
                # Handle control button clicks
                    ...
                    self.randomize_beat()
                elif event.ui_element == self.demo_random_button:
                    self.generate_demo()
                elif event.ui_element == self.play_demo_button:
                    self.play_demo()
                elif event.ui_element == self.stop_demo_button:
                    self.stop_demo()
                elif event.ui_element == self.check_button:
                    self.check_user_beat()
    ...
    def play_spooky_background(self):
        """Play the spooky melody that matches the selected demo number."""
        self.current_spooky = self.spooky_melodies[self.selected_demo]
        print(f"Playing spooky melody {self.selected_demo + 1}")
        self.current_spooky.play(loops=-1)
    ...
    def generate_demo(self):
        """Pick a random demo track number (1–3) and reset play count."""
        self.selected_demo = random.randint(0, 2)
        self.demo_play_count = 0
        self.demo_label.set_text(f"Demo: {self.selected_demo + 1} (0/3 plays)")
        print(f"Selected demo: {self.selected_demo + 1}")

    def play_demo(self):
        """Play the selected pre-mixed demo track (max 3 plays)."""
        if self.selected_demo is None:
            self.demo_label.set_text("Select a demo first!")
            return

        # Check play limit
        if self.demo_play_count >= self.max_demo_plays:
            self.demo_label.set_text("Max plays reached! Randomize again.")
            print("Reached max plays for this demo.")
            return

        try:
            pygame.mixer.music.load(self.demo_tracks[self.selected_demo])
            pygame.mixer.music.play()
        except Exception as e:
            print(f"Error playing demo track: {e}")
            return

        self.demo_play_count += 1
        self.demo_label.set_text(f"Demo: {self.selected_demo + 1} ({self.demo_play_count}/3 plays)")
        print(f"Playing demo {self.selected_demo + 1} ({self.demo_play_count}/3)")

    def check_user_beat(self):
        """Compare user’s beat matrix with the correct demo pattern."""
        if self.selected_demo is None:
            self.demo_label.set_text("Select a demo first!")
            return

        correct = self.correct_patterns[self.selected_demo]
        user = self.beat_matrix

        # Compare all instruments and steps
        matches = all(
            user[i][j] == correct[i][j]
            for i in range(3)
            for j in range(8)
        )

        if matches:
            self.demo_label.set_text("✅ Correct beat! Great job!")
            print("User beat matches the demo pattern!")
        else:
            self.demo_label.set_text("❌ Incorrect. Try again!")
            print("User beat does not match the demo pattern.")
